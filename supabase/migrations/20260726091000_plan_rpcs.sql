...
-- ---------------------------------------------------------------------
-- Writes the log row with content SNAPSHOTTED from the activity library at
-- this moment. Doing it server-side means the snapshot can't be forged or
-- drift from what was actually recommended.
...
begin
  ...
  insert into activity_log (
    child_id, activity_id, domain, title, age_band, plan_date, note
  )
  values (
    p_child_id, v_a.id, v_a.domain, v_a.title, v_a.age_band,
    plan_date_for(p_child_id), p_note
  )
